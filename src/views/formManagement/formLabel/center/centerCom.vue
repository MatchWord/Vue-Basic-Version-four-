<template>
  <div class="form-edit" :style="{backgroundImage: 'url(' + bgPic + ')' }">
    <form class="published-form">
      <div class="form-body" v-if="newFormListData.length">
        <div class="ant-row">
              ...
              <div class="fieldDraggableWrapper" v-for="(element, index) in newFormListData" :key="element.order">
                <div :class="[element.isSelect * 1 === 1 ? 'active' : '', 'field-wrapper']" @click="elementClick(element, index)">
                  <div class="field-container">
                    <div class="ant-form-item" v-if="element.title || element.titlePlaceholder">
                    ...
                    <div class="ant-form-item-control-wrapper">
                      ...
                      <div class="ant-form-item-control" v-if="element.type === 'edit'">
                        <span class="ant-input-affix-wrapper" :style="{ width: (element.widthPos * 100) + '%' }">
                          <tinymce v-model="element.content" :value="element.content" :height="200" />
                        </span>
                      </div>
                    </div>
                  </div>
                  <div class="actions">
                    <div class="actions_position">
                      <el-button class="btn-ud" icon="el-icon-upload2" size="mini" @click="moveItem(element, 'up')"></el-button>
                      <el-button class="btn-ud" icon="el-icon-download" size="mini" @click="moveItem(element, 'down')"></el-button>
                    </div>
                    <div class="actions_mutation">
                      <el-button type="primary" icon="el-icon-document-copy" circle @click="copyItem(element, index)"></el-button>
                      <el-button type="danger" icon="el-icon-delete" circle @click="delectItem(element, index)"></el-button>
                      </div>
                  </div>
                </div>
              </div>
